$(function () {
    ...
    function addRow(id, login_id, user_name, timestamp, edited_user) {
        ...
        var checkboxCell = $('<td></td>').addClass('px-6 py-4')
            .append($('<input>')
                .attr('type', 'checkbox')
        ...
        row.append($('<td></td>').addClass('px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white').text(id));
        row.append($('<td></td>')
            .addClass('p-3 text-sm text-blue-300 cursor-pointer edit_user')
            .text(login_id))
            .attr('data-id', id); // Add data-id attribute to the td
        row.append($('<td></td>').addClass('px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white').text(user_name));
        row.append($('<td></td>').addClass('px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white').text(timestamp));
        row.append($('<td></td>').addClass('px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white').text(edited_user));
    ...
    $(document).on('click', '.edit_user', function () {
        ...
        axios.get('/api/backend/admin/tenent_users/' + user_id + '?tenant=' + tenant)
            .then(response => {
                ...
                Swal.fire({
                    ...
                    html:
                        ...
                        '<label for="loginId" class="block text-sm font-medium text-gray-700 mb-2">' + langs.login_id + '</label>' +
                        '<input id="loginId" type="text" class="swal2-input w-64 p-2 border border-gray-300 rounded-md" placeholder="' + langs.login_id + '"  value="' + usersData.login_id + '" required>' +
                        '</div>' +
                        '<div class="mb-4 text-left">' +
                        '<label for="password" class="block text-sm font-medium text-gray-700 mb-2">' + langs.password + '</label>' +
                        '<input id="password" type="password" class="swal2-input w-64 p-2 border border-gray-300 rounded-md" placeholder="' + langs.password + '" required>' +
                        '</div>' +
                        '<div class="mb-4 text-left">' +
                        '<label for="userName" class="block text-sm font-medium text-gray-700 mb-2">' + langs.user_name + '</label>' +
                        '<input id="userName" type="text" class="swal2-input w-64 p-2 border border-gray-300 rounded-md" placeholder="' + langs.user_name + '" value="' + usersData.user_name + '" required>' +
                        '</div>',
                    focusConfirm: false,
                    ...
                    didOpen: () => {
                        ...
                        if (confirmButton) {
                            confirmButton.addEventListener('click', async () => {
                                ...
                                var data = { login_id: loginId, password: password, user_name: userName, _method: "PUT", tenant: tenant };
                                try {
                                    const response = await axios.post(`/api/backend/admin/tenent_users/${user_id}/update`, data);
        ...
        axios.get(api)
            .then(response => {
                ...
                usersData.forEach(user => {
                    addRow(user.id, user.login_id, user.user_name, user.updated_at, user?.updated_by?.user_name);
                });
                // Pagination details
        ...
        if (selectedIds.length > 0) {
            Swal.fire({
                ...
                customClass: {
                    ...
                    cancelButton: 'bg-gray-300 text-black hover:bg-gray-400 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2',
                },
            }).then(async (result) => {
                if (result.isConfirmed) {
    ...
    $('#addUserBtn').on('click', function () {
        Swal.fire({
            ...
            html:
                '<div class="mb-4 text-left">' +
                '<label for="loginId" class="block text-sm font-medium text-gray-700 mb-2">' + langs.login_id + '</label>' +
                '<input id="loginId" type="text" class="swal2-input w-64 p-2 border border-gray-300 rounded-md" placeholder="' + langs.login_id + '" required>' +
                '</div>' +
                '<div class="mb-4 text-left">' +
                '<label for="password" class="block text-sm font-medium text-gray-700 mb-2">' + langs.password + '</label>' +
                '<input id="password" type="password" class="swal2-input w-64 p-2 border border-gray-300 rounded-md" placeholder="' + langs.password + '" required>' +
                ...
                '<div class="mb-4 text-left">' +
                '<label for="userName" class="block text-sm font-medium text-gray-700 mb-2">' + langs.user_name + '</label>' +
                '<input id="userName" type="text" class="swal2-input w-64 p-2 border border-gray-300 rounded-md" placeholder="' + langs.user_name + '" required>' +
                '</div>',
            focusConfirm: false,
            showCancelButton: true,
            confirmButtonText: langs.yes,
            ...
            didOpen: () => {
                ...
                if (confirmButton) {
                    confirmButton.addEventListener('click', async () => {
                        ...
                        // If validation passes, send the API request
                        var data = { login_id: loginId, password: password, user_name: userName, tenant: tenant };
                        try {
                            const response = await axios.post('/api/backend/admin/tenent_users', data);
