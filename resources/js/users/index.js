$(function() {
    ...
    $(document).on('click', '.edit_admin', function() {
        ...
        axios.get('/api/backend/admin/users/'+user_id)
        .then(response => {
                ...
                didOpen: () => {
                    ...
                    if (confirmButton) {
                        confirmButton.addEventListener('click', async () => {
                            ...
                            // If validation passes, send the API request
                            var data = { login_id: loginId, password: password, user_name: userName, _method : "PUT" };
                            try {
                                const response = await axios.post(`/api/backend/admin/users/${user_id}/update`, data);
    ...
    function addRow(id, login_id, user_name, timestamp, edited_user) {
        ...
        row.append($('<td></td>')
        .addClass('p-3 text-sm text-blue-300 cursor-pointer edit_admin')
        .text(login_id))
